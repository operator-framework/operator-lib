...
var _ = Describe("filter", func() {
	...
	Context("Falsy", func() {
		...
		BeforeEach(func() {
			pred, err = annotation.NewFalsyPredicate[client.Object](annotationKey, annotation.Options{Log: logf.Log})
			Expect(err).NotTo(HaveOccurred())
			hdlr, err = annotation.NewFalsyEventHandler[client.Object](annotationKey, annotation.Options{Log: logf.Log})
			Expect(err).NotTo(HaveOccurred())
		})
		...
		Context("Update", func() {
			...
			Context("returns true", func() {
				...
				It("receives neither objects having any annotations", func() {
					old, updated := pod.DeepCopy(), pod.DeepCopy()
					old.SetLabels(map[string]string{"id": "old"})
					updated.SetLabels(map[string]string{"id": "updated"})
					e = makeUpdateEventFor(old, updated)
					Expect(pred.Update(e)).To(BeTrue())
					hdlr.Update(ctx, e, q)
					verifyQueueHasPod(q, updated)
				})
				It("receives the new object with a registered key and false value", func() {
					old, updated := pod.DeepCopy(), pod.DeepCopy()
					old.SetLabels(map[string]string{"id": "old"})
					updated.SetLabels(map[string]string{"id": "updated"})
					updated.SetAnnotations(map[string]string{annotationKey: "false"})
					e = makeUpdateEventFor(old, updated)
					Expect(pred.Update(e)).To(BeTrue())
					hdlr.Update(ctx, e, q)
					verifyQueueHasPod(q, updated)
				...
				It("receives the new object with a non-registered key and true value", func() {
					...
					old.SetLabels(map[string]string{"id": "old"})
					updated.SetLabels(map[string]string{"id": "updated"})
					updated.SetAnnotations(map[string]string{"my.app/foo": "true"})
					e = makeUpdateEventFor(old, updated)
					Expect(pred.Update(e)).To(BeTrue())
					hdlr.Update(ctx, e, q)
					verifyQueueHasPod(q, updated)
				...
				It("receives the new object with a non-registered key and false value", func() {
					...
					old.SetLabels(map[string]string{"id": "old"})
					updated.SetLabels(map[string]string{"id": "updated"})
					updated.SetAnnotations(map[string]string{"my.app/foo": "false"})
					e = makeUpdateEventFor(old, updated)
					Expect(pred.Update(e)).To(BeTrue())
					hdlr.Update(ctx, e, q)
					verifyQueueHasPod(q, updated)
			...
			Context("returns false", func() {
				It("receives the new object with a registered key", func() {
					old, updated := pod.DeepCopy(), pod.DeepCopy()
					updated.SetAnnotations(map[string]string{annotationKey: "true"})
					e = makeUpdateEventFor(old, updated)
	...
	Context("Truthy", func() {
		...
		BeforeEach(func() {
			pred, err = annotation.NewTruthyPredicate[client.Object](annotationKey, annotation.Options{Log: logf.Log})
			Expect(err).NotTo(HaveOccurred())
			hdlr, err = annotation.NewTruthyEventHandler[client.Object](annotationKey, annotation.Options{Log: logf.Log})
			Expect(err).NotTo(HaveOccurred())
		})
